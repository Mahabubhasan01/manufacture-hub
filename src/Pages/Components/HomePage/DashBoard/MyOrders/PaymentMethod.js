  ...
  if (isLoading) {
    ...
    return (
        ...
        <div class="flex justify-between mt-10 mb-5">
          <span class="font-semibold text-sm uppercase">Items {orders.length} <span className="text-xl"></span></span>
          
            {orders.slice(0,1).map(order=><span key={order._id}>${order.price*order.quantity*orders.length}</span>)}
        </div>
        <div>
